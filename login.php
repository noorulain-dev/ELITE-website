...
<header id="aboutheader">	
	...
	if(isset($_POST['submit_btn'])){
        ...
        $loginInfo = "$nameLogin,$emailLogin,$passwordLogin";
        $userDetails = file_get_contents($emailLogin . ".txt");

        if (stristr($userDetails,$loginInfo)) {
			$_SESSION["username"] = $nameLogin;
			$_SESSION["e-mail"] = $emailLogin;
			$_SESSION["pass"]= $passwordLogin;
            echo("<script>window.location = 'viewAccount.php';</script>");
        }

        else {
            echo "<br> You have entered the wrong email or password. Please try again. <br>";
            echo("<script>window.location = 'loginfail.html';</script>");
        }
	...
	<div class="b_container">
		...
		<figure class="page_banner">
			...
			<p id="banner_text"> Login </p>
			
		</figure>
	</div>

	<h1 class="registerh"> Login Here </h1>


	<p class="center"> Please enter your email and password below: </p>
	...
	<form action="login.php" method="post">
		...
		<legend class="loginuser"> Login </legend>
        <p>
	...
	<button type="submit" name="submit_btn" value="Submit" class="zoom"> Login</button>
	<button type="reset" value="reset" class="zoom"> Cancel</button>
</div>
	
	<p> No account? Register <a href="register.php" class="ext_link"> here </a></p>
